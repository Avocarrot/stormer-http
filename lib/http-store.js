...
class HttpStore extends Store {
   ...
   */
  _get(model, pk) {
    let method = this._method('get');
    let url    = this._path('get', model, { pk });
   ...
   */
  _filter(model, query) {
    let method = this._method('filter');
    let url    = this._path('filter', model, query);
    return this._requestPromise({method, url, qs: query, useQuerystring: true}).then((res) => {
  ...
  _set(model, data, operation) {
    ...
    let method = this._method(operation);
    let url    = this._path(operation, model, data);
    return this._requestPromise({method, url, formData: data}).then((res) => {
      let recordId = data[model.schema.primaryKey];
   ...
   */
  _delete(model, query) {
    let method = this._method('delete');
    let url    = this._path('delete', model, query);
    return this._requestPromise({method, url }).then((res) => {
      if (res.statusCode >= 400) {
