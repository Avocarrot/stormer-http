  ...
  _get(model, pk) {
    ...
    return this._requestPromise({method, url}).then((res) => {
      if (res.statusCode === 404) {
        return Promise.reject(new HttpResponseError({ message: `Could not find "${model.name}" with primary key "${pk}"`}, res.statusCode));
      }
      if (res.statusCode >= 400) {
        return Promise.reject(new HttpResponseError(res.body, res.statusCode));
      }
