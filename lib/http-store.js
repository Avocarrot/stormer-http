...
class HttpStore extends Store {
  ...
  _filter(model, query) {
    return this._requestPromise({
      method: 'GET',
      url: util.format('%s/%s?query=%s', this._baseUrl, model.name, JSON.strinfigy(query))
    });
  }
   ...
   * @memberof HttpStore
   * @param {Object} model - The model
   * @param {Object} formData - The data payload to push
   * @param {String} operation - The operation to perform
   * @param {String} pk - The object's primary key
   * @return {Promise}
   *
   */
  _set(model, formData, operation, pk) {
    let method;
    let url;
    switch (operation) {
      case 'create':
        method = 'POST';
        url = util.format('%s/%s', this._baseUrl);
        break;
      case 'update':
      default:
        method = 'PATCH';
    ...
    return this._requestPromise({method, url, formData});
  }
