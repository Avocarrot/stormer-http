...
class HttpStore extends Store {
   ...
   */
  _get(model, pk) {
    let method = this._methodFor('get');
    let url    = this._pathFor('get', model, { pk });
   ...
   */
  _filter(model, query) {
    let method = this._methodFor('filter');
    let url    = this._pathFor('filter', model, query);
  ...
  _set(model, data, operation) {
    ...
    let method = this._methodFor(operation);
    let url    = this._pathFor(operation, model, data);
   ...
   */
  _delete(model, query) {
    let method = this._methodFor('delete');
    let url    = this._pathFor('delete', model, query);
    return this._requestPromise({method, url }).then((res) => {
      if (res.statusCode >= 400) {
